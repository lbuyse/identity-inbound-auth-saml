    ...
    public SAMLSSOReqValidationResponseDTO process(LogoutRequest logoutRequest, String sessionId,
        ...
        try {
            ...
            if (logoutRequest != null) {
                ...
                // Session index should be present in the logout request in SP init logout scenario.
                String sessionIndex = logoutRequest.getSessionIndexes().size() > 0 ?
                        logoutRequest.getSessionIndexes().get(0).getSessionIndex() : null;

                if (StringUtils.isBlank(sessionIndex)) {
                    String message = "Session index was not present in the logout request.";
                    log.error(message);
